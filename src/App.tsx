...
const LandingApp: React.FC = () => {
  ...
  const { brand, nav, hero, story, services, products, contact, footer } = content;
      ...
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white shadow-md py-3' : 'bg-transparent py-5'}`}>
        <div className="container mx-auto px-4 flex items-center justify-between">
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
            {brand.logoImage ? (
              <img src={brand.logoImage} alt={brand.name} className="h-10 w-auto object-contain" />
            ) : (
              <div className="w-10 h-10 bg-brand-green rounded-full flex items-center justify-center text-white font-bold text-xl">{brand.logo}</div>
            )}
            <span className="text-xl font-bold tracking-tight text-brand-green">{brand.name}</span>
          </div>
          ...
          <div className="flex items-center gap-4">
            <div className="relative group cursor-not-allowed opacity-50">
              ...
              <span className="absolute -top-2 -right-2 bg-brand-red text-white text-[10px] w-4 h-4 rounded-full flex items-center justify-center">{cart.length}</span>
            </div>
      ...
      <main>
              ...
              <h1 className="text-4xl md:text-6xl font-bold text-brand-green leading-tight mb-6">
                {hero.titleLines.map((line, idx) => (
                  <React.Fragment key={idx}>
                    <span className={hero.highlightedLines.includes(idx) ? 'text-brand-red' : undefined}>{line}</span>
        ...
        <section id="services" className="py-20 bg-brand-bg">
          <div className="container mx-auto px-4">
            <div className="text-center mb-16">
              <h2 className="text-3xl md:text-4xl font-bold text-brand-green mb-4">{services.title}</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {services.items.map((service, idx) => (
                <div key={idx} className="bg-white p-8 rounded-2xl shadow-sm hover:shadow-md transition-all border border-gray-100 group">
                  ...
                  <h3 className="text-xl font-bold text-brand-green mb-4">{service.title}</h3>
        ...
        <section id="products" className="py-20 bg-white">
          <div className="container mx-auto px-4">
            <div className="flex flex-col md:flex-row items-end justify-between mb-12 gap-6">
              <div>
                <h2 className="text-3xl md:text-4xl font-bold text-brand-green mb-4">{products.title}</h2>
        ...
        <section id="contact" className="py-20 bg-brand-green text-white">
          <div className="container mx-auto px-4 text-center">
            <h2 className="text-3xl md:text-4xl font-bold mb-8">{contact.title}</h2>
        ...
        <div className="container mx-auto px-4">
          <div className="flex flex-col md:flex-row justify-between items-center gap-8">
            <div className="flex items-center gap-2">
              {brand.logoImage ? (
                <img src={brand.logoImage} alt={brand.name} className="h-8 w-auto object-contain" />
              ) : (
                <div className="w-8 h-8 bg-brand-green rounded-full flex items-center justify-center text-white font-bold text-sm">{brand.logo}</div>
              )}
              <span className="text-lg font-bold text-brand-green">{brand.name}</span>
            </div>
